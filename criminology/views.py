...
from django.db.models import Case, Count, F, IntegerField, Max, OuterRef, Subquery, Sum, Value, Avg, Q, F, FloatField, ExpressionWrapper
...
def homepage(request):
    ...
    if active_year:
        ...
        for record in assessment_records:
            summary = {
                ...
                'evaluating': round(record.calculate_evaluating_percentage, 2),
                'average': round(record.calculate_overall_percentage, 2),
                'total_questions': record.category_total,
            }
            assessment_summaries.append(summary)
    ...
    # ========== NEW QUESTION-BASED INSIGHTS ==========
    
    # 1. Question Performance Analysis (Most Challenging & Easiest)
    question_performance = []
    if active_year:
        # Get all questions used in assessments with correct/wrong counts
        question_stats = CorrectAndWrongCountsPerItem.objects.filter(
            assessment_id__in=[rec.assessment_id for rec in assessment_records]
        )
        
        for stat in question_stats:
            total_attempts = stat.correct_counts + stat.wrong_counts
            if total_attempts > 0:
                difficulty = round((stat.correct_counts / total_attempts) * 100, 2)
                question_performance.append({
                    'question': stat.question_description[:50] + '...' if len(stat.question_description) > 50 else stat.question_description,
                    'number': stat.number,
                    'assessment_id': stat.assessment_id,
                    'correct': stat.correct_counts,
                    'wrong': stat.wrong_counts,
                    'difficulty': difficulty,
                })
    
    # Sort and get top 5 easiest and hardest
    question_performance.sort(key=lambda x: x['difficulty'])
    hardest_questions = question_performance[:5]  # Lowest difficulty %
    easiest_questions = question_performance[-5:][::-1]  # Highest difficulty %
    ...
    if active_year:
        for record in assessment_records:
            students_completed = StudentsScoreAssessment.objects.filter(
                academic_year=active_year,
                exam_id=record.assessment_id
            ).count()
            
            completion_rate = round((students_completed / enrolled_students_count) * 100, 2) if enrolled_students_count > 0 else 0
            
            assessment_completion.append({
                'assessment_id': record.assessment_id,
                'topic': record.topic or f"Assessment {record.assessment_id}",
                'subject_code': record.subject_code,
                'students_completed': students_completed,
                'completion_rate': completion_rate,
            })

    # 4. Pass/Fail Rate per Assessment (assuming 60% is passing)
    pass_fail_data = []
    if active_year:
        for record in assessment_records:
            scores = StudentsScoreAssessment.objects.filter(
                academic_year=active_year,
                exam_id=record.assessment_id
            )
            ...
            if total_students > 0:
                ...
                passed = sum(1 for s in scores if s.total_questions_taken > 0 and (s.total_questions_score / s.total_questions_taken * 100) >= 60)
    ...
    # 5. Questions by Status (Active vs Restricted)
    question_status_summary = {
        'active': questionnaires.filter(status=0).count(),
        'restricted': restricted_counts,
        'total': total_questionnaires,
    }

    # 6. Recently Added Questions
    recent_questions = list(
        questionnaires.filter(status=0)
        .select_related('subject', 'category', 'topic')
        .order_by('-id')[:5]
    )

    # 7. Subject Question Bank Status
    subject_question_bank = []
    for subject in Subject.objects.all():
        total_q = questionnaires.filter(subject=subject).count()
        active_q = questionnaires.filter(subject=subject, status=0).count()
        restricted_q = questionnaires.filter(subject=subject, status=1).count()
        
        if total_q > 0:
            subject_question_bank.append({
                'subject_code': subject.subject_code,
                'subject_name': subject.subject_name,
                'total': total_q,
                'active': active_q,
                'restricted': restricted_q,
                'active_percentage': round((active_q / total_q) * 100, 2),
            })
    
    subject_question_bank.sort(key=lambda x: x['total'], reverse=True)

    # 8. High-P and Low-P Questions Summary
    high_p_questions = []
    low_p_questions = []
    if active_year:
        high_p = HighPQuestionsReports.objects.filter(
            assessment_id__in=[rec.assessment_id for rec in assessment_records]
        ).order_by('-percentage')[:10]
        
        low_p = LowPQuestionsReports.objects.filter(
            assessment_id__in=[rec.assessment_id for rec in assessment_records]
        ).order_by('percentage')[:10]
        
        high_p_questions = [{'question': h.high_p_q, 'number': h.number, 'percentage': h.percentage, 'assessment_id': h.assessment_id} for h in high_p]
        low_p_questions = [{'question': l.low_p_q, 'number': l.number, 'percentage': l.percentage, 'assessment_id': l.assessment_id} for l in low_p]
    ...
    context = {
        ...
        'question_status_summary': question_status_summary,
        'recent_questions': recent_questions,
        'subject_question_bank': subject_question_bank,
        'high_p_questions': high_p_questions,
        'low_p_questions': low_p_questions,
    }
